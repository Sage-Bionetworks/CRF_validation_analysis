...
getStairStartAndStopTime <- function(stairJsonFileLoc){
  ...
  if(c('stepPath') %in% colnames(dat)){
    dat <- dat %>%
      dplyr::filter(stepPath == 'Cardio Stair Step/stairStep/motion')
  }
  ...
  return(list(stairStartTime = startTime,
              stairStopTime = stopTime))
}
...
stair.times <- apply(hr.table.meta,1,function(x){ 
  ...
  dplyr::select(recordId, stairStartTime, stairStopTime) 
...
thisFileName <- 'analysis/crf_stairStep_start_stop.R'
